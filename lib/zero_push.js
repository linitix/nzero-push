      ...
      function (next) {
        ...
        switch (platform) {
          ...
          case SAFARI:
            ...
            break;
          case ANDROID:
            typeof optionalParams === "function" ? self._notifyAndroidPlatform(requiredParams, next) : self._notifyAndroidPlatform(requiredParams, optionalParams, next);
            break;
          default:
...
ZeroPush.prototype._notifyAndroidPlatform = function (requiredParams, optionalParams, callback) {
  debug("PRIVATE ENDPOINT: notifyAndroidPlatform");
  ...
  if (typeof requiredParams.data !== "object") {
    return callback(new ClientError("Parameter Type Error", "\"data\" need to be a JSON"));
  }

  requestURL = self.baseURL + ENDPOINTS.notify;
  ...
  body.device_tokens = requiredParams.device_tokens;
  body.data = requiredParams.data;

  if (optionalParams && typeof optionalParams === "object") {
    if (optionalParams.collapse_key) {
      body.collapse_key = optionalParams.collapse_key;
    }

    if (optionalParams.delay_while_idle) {
      body.delay_while_idle = optionalParams.delay_while_idle;
    }

    if (optionalParams.time_to_live) {
      body.time_to_live = optionalParams.time_to_live;
    }
  }
      ...
      function (next) {
        ...
        switch (platform) {
          ...
          case SAFARI:
            ...
            break;
          case ANDROID:
            typeof optionalParams === "function" ? self._broadcastAndroidPlatform(requiredParams, next) : self._broadcastAndroidPlatform(requiredParams, optionalParams, next);
            break;
          default:
            next(new UnknownPlatformTypeError("Unknown platform type! Please chose between ios-mac or safari!"));
            break;
...
ZeroPush.prototype._broadcastAndroidPlatform = function (requiredParams, optionalParams, callback) {
  debug("PRIVATE ENDPOINT: broadcastAndroidPlatform");
...
ZeroPush.prototype.retrieveSetOfChannels = function (page, channelsPerPage, callback) {
  debug("ENDPOINT: GET /channels");

  var self = this;
  var requestURL = self.baseURL + ENDPOINTS.channels;
  var options = clone(self.baseRequestOptions);
  var requestUrlModified = false;

  if (typeof page === "function") {
    callback = page;
  } else if (typeof channelsPerPage === "function") {
    callback = channelsPerPage;
  }
  ...
  if (typeof channelsPerPage === "number" && channelsPerPage >= 1) {
    requestUrlModified ? requestURL += "&per_page=" + channelsPerPage : requestURL += "?per_page=" + channelsPerPage;
  }
  ...
  executeRequest(
    ...
    function (err, body, headers) {
      ...
      var response = {
        devices   : body,
        total     : headers.total || headers.Total || null,
        pagination: linksParser(headers.link)
      };
