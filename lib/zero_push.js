    ...
    function (err, res, body) {
      ...
      parseResponse(
        ...
        function (err, parsedBody) {
          ...
          if (!parsedBody.error)
            return callback(null, parsedBody, res.headers);

          switch (parsedBody.error.toLowerCase()) {
            case "authorization error":
              callback(new AuthorizationError(parsedBody.message, parsedBody.reference_url));
              break;
            case "quota error":
              callback(new QuotaError(parsedBody.message, parsedBody.reference_url));
              break;
            case "configuration error":
              callback(new ConfigurationError(parsedBody.message, parsedBody.reference_url));
              break;
            default:
              callback(new ClientError(parsedBody.error.toLowerCase(), parsedBody.message, parsedBody.reference_url));
              break;
          }
        }
      );
